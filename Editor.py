...
# Get the first video file from the second input folder
video_files2 = [
...
for video_file1 in video_files1:
    # random video file from the second folder
    video_file2 = random.choice(video_files2)
    ...
    p = 2  # int(input(f"How many parts would you like {video_file1} to be divided into? "))
    ...
    # Create the video clip for the first file in the second folder
    clip2 = VideoFileClip(input_video_path2).subclip(0, 0 + length2)

    # make clip2 loop with the duration of clip1
